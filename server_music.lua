...
-- Usamos SoundService como contenedor principal para la música de fondo
...
local function playMusicInLoop(musicList)
    ...
    while true do
        ...
        for _, id in ipairs(shuffledList) do
            ...
            sound:Play()
            -- sound.Ended:Wait() espera a que el evento de fin de sonido se dispare
            local success, message = pcall(function() sound.Ended:Wait() end)
    ...
    local function realTimeMusicManager()
        while true do
            ...
            if themeName ~= currentTheme then
                ...
                -- La fecha no ha cambiado, la música sigue sonando en su propia tarea (currentMusicTask)
            end
